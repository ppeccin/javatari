...
import general.m6502.instructions.Illegal;
...
public final class M6502 implements ClockDriven {
	...
	/** This version executes all fetch operations on the FIRST cycle, 
	...
	public char fetchZeroPageXAddress() {
		return (char) toUnsignedByte(memory.readByte(PC++) + X);		// Sum should wrap the byte and allways be in range 0 - ff
	}

	public int fetchZeroPageYAddress() {
		return (char) toUnsignedByte(memory.readByte(PC++) + Y);		// Sum should wrap the byte and allways be in range 0 - ff
	}
	...
	public final Instruction ILLEGAL = new Illegal(this);
